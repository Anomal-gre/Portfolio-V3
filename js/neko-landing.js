/* ── Neko Landing — Autonomous Pixel Cat with Drag & Drop & Tap-to-Stop Toggle ── */
(function () {
  'use strict';

  // 32 Original Neko98 32x32 Sprites (Base64)
  const SPRITES = [
    "data:image/png;base64,", // 0: AWAKE (!)
    "data:image/png;base64,", // 1: U_MOVE 1
    "data:image/png;base64,", // 2: U_MOVE 2
    "data:image/png;base64,", // 3: UR_MOVE 1
    "data:image/png;base64,", // 4: UR_MOVE 2
    "data:image/png;base64,", // 5: R_MOVE 1
    "data:image/png;base64,", // 6: R_MOVE 2
    "data:image/png;base64,", // 7: DR_MOVE 1
    "data:image/png;base64,", // 8: DR_MOVE 2
    "data:image/png;base64,", // 9: D_MOVE 1
    "data:image/png;base64,", // 10: D_MOVE 2
    "data:image/png;base64,", // 11: DL_MOVE 1
    "data:image/png;base64,", // 12: DL_MOVE 2
    "data:image/png;base64,", // 13: L_MOVE 1
    "data:image/png;base64,", // 14: L_MOVE 2
    "data:image/png;base64,", // 15: UL_MOVE 1
    "data:image/png;base64,", // 16: UL_MOVE 2
    "data:image/png;base64,", // 17: U_CLAW 1 (Dangling 1)
    "data:image/png;base64,", // 18: U_CLAW 2 (Dangling 2)
    "data:image/png;base64,", // 19: R_CLAW 1
    "data:image/png;base64,", // 20: R_CLAW 2
    "data:image/png;base64,", // 21: L_CLAW 1
    "data:image/png;base64,", // 22: L_CLAW 2
    "data:image/png;base64,", // 23: D_CLAW 1
    "data:image/png;base64,", // 24: D_CLAW 2
    "data:image/png;base64,", // 25: WASH
    "data:image/png;base64,", // 26: SCRATCH 1
    "data:image/png;base64,", // 27: SCRATCH 2
    "data:image/png;base64,", // 28: STOP (Sitting)
    "data:image/png;base64,", // 29: YAWN
    "data:image/png;base64,", // 30: SLEEP 1
    "data:image/png;base64,"  // 31: SLEEP 2 (Zzz)
  ];

  // Authentic Neko98 Direction Sprite Map
  const MOVE_SPRITES = {
    U:  [1, 2],    // Up
    UR: [3, 4],    // Up-Right
    R:  [5, 6],    // Right
    DR: [7, 8],    // Down-Right
    D:  [9, 10],   // Down
    DL: [11, 12],  // Down-Left
    L:  [13, 14],  // Left
    UL: [15, 16]   // Up-Left
  };

  // Dangling / Lifted paw sprites when being dragged
  const DRAG_SPRITES = [17, 18];

  class NekoLanding {
    constructor() {
      this.landing = document.getElementById('landing');
      if (!this.landing) return;

      this.headline = this.landing.querySelector('.hero-headline');
      this.heroStack = this.landing.querySelector('.hero-stack');

      // Create Neko container
      this.el = document.createElement('div');
      this.el.id = 'neko-landing';
      this.el.className = 'neko-cat';

      // Interactive tooltip notice
      this.tooltip = document.createElement('div');
      this.tooltip.className = 'neko-tooltip';
      this.tooltip.setAttribute('data-i18n', 'neko_tooltip');
      const savedLang = localStorage.getItem('site_lang') || 'en';
      this.tooltip.textContent = savedLang === 'vi' ? 'Click hoặc giữ' : 'Click or hold & drag';
      this.el.appendChild(this.tooltip);

      this.img = document.createElement('img');
      this.img.src = SPRITES[28]; // Initial sitting sprite
      this.img.alt = 'Neko Cat';
      this.img.draggable = false;
      this.el.appendChild(this.img);

      this.landing.appendChild(this.el);

      // State
      this.x = 0;
      this.y = 0;
      this.targetX = 0;
      this.targetY = 0;
      this.speed = 1.28; // Movement speed
      this.direction = 'D';
      this.state = 'IDLE'; // 'WALKING', 'IDLE', 'AWAKE', 'DRAGGING'
      this.idleType = 'STOP'; // 'STOP', 'WASH', 'SCRATCH', 'YAWN', 'SLEEP'
      this.idleTimer = 0;
      this.animTick = 0;
      this.frameToggle = 0;
      this.isVisible = true;
      this.isPaused = false; // Toggle state: permanently stop or roam

      // Pointer / Drag State
      this.isDragging = false;
      this.pointerStartX = 0;
      this.pointerStartY = 0;

      // Init position
      this.updateBounds();
      this.x = this.bounds.minX + Math.random() * (this.bounds.maxX - this.bounds.minX);
      this.y = this.bounds.minY + Math.random() * (this.bounds.maxY - this.bounds.minY);
      this.targetX = this.x;
      this.targetY = this.y;
      this.updateElementPosition();

      // Events
      window.addEventListener('resize', () => this.updateBounds(), { passive: true });
      this.setupInteraction();

      // Pause loop when out of view to save CPU
      const observer = new IntersectionObserver((entries) => {
        this.isVisible = entries[0].isIntersecting;
      }, { threshold: 0.1 });
      observer.observe(this.landing);

      // Start loop
      this.lastTime = performance.now();
      this.pickNewTarget();
      requestAnimationFrame((t) => this.loop(t));
    }

    setupInteraction() {
      const onPointerMove = (e) => {
        const dx = e.clientX - this.pointerStartX;
        const dy = e.clientY - this.pointerStartY;
        const dist = Math.hypot(dx, dy);

        if (!this.isDragging && dist > 6) {
          // Began dragging
          this.isDragging = true;
          this.state = 'DRAGGING';
          this.el.classList.add('is-dragging');
        }

        if (this.isDragging) {
          const landingRect = this.landing.getBoundingClientRect();
          // Center cat on pointer (36px offset for 72px cat)
          this.x = e.clientX - landingRect.left - 36;
          this.y = e.clientY - landingRect.top - 36;

          // Clamp within landing bounds
          this.x = Math.max(this.bounds.minX, Math.min(this.bounds.maxX, this.x));
          this.y = Math.max(this.bounds.minY, Math.min(this.bounds.maxY, this.y));

          this.targetX = this.x;
          this.targetY = this.y;
          this.updateElementPosition();

          // Dangling / clawing animation while being dragged
          const dragFrameIndex = Math.floor(this.animTick / 10) % 2;
          this.img.src = SPRITES[DRAG_SPRITES[dragFrameIndex]];
        }
      };

      const onPointerUp = () => {
        window.removeEventListener('pointermove', onPointerMove);
        window.removeEventListener('pointerup', onPointerUp);
        window.removeEventListener('pointercancel', onPointerUp);

        if (this.isDragging) {
          // Finished dragging & dropped at new position
          this.isDragging = false;
          this.el.classList.remove('is-dragging');
          this.targetX = this.x;
          this.targetY = this.y;

          if (this.isPaused) {
            // Stay paused at new location
            this.state = 'IDLE';
            this.idleType = 'STOP';
            this.img.src = SPRITES[28];
          } else {
            // Alert on landing, then resume
            this.state = 'AWAKE';
            this.img.src = SPRITES[0]; // '!'
            this.idleTimer = 25;
          }
        } else {
          // Single tap / click -> Toggle Stop & Move!
          if (!this.isPaused) {
            // Pause: Stop and sit still permanently!
            this.isPaused = true;
            this.state = 'IDLE';
            this.idleType = 'STOP';
            this.img.src = SPRITES[28]; // Sitting sprite
            this.targetX = this.x;
            this.targetY = this.y;
            this.el.setAttribute('title', 'Đang dừng 🐾 Nhấn lần nữa để mèo đi tiếp');
          } else {
            // Resume: Wake up and move normally!
            this.isPaused = false;
            this.wakeUp();
            this.el.setAttribute('title', 'Nhấn để dừng lại, giữ & kéo để di chuyển 🐾');
          }

          // Trigger bounce effect on img
          this.el.classList.add('is-petted');
          setTimeout(() => this.el.classList.remove('is-petted'), 350);
        }
      };

      this.el.addEventListener('pointerdown', (e) => {
        e.preventDefault();
        this.pointerStartX = e.clientX;
        this.pointerStartY = e.clientY;

        window.addEventListener('pointermove', onPointerMove, { passive: false });
        window.addEventListener('pointerup', onPointerUp);
        window.addEventListener('pointercancel', onPointerUp);
      });
    }

    updateBounds() {
      if (!this.landing) return;
      const landingRect = this.landing.getBoundingClientRect();
      const pad = 24;
      const catSize = 72;

      let topLimit = pad;
      let bottomLimit = landingRect.height - catSize - pad;
      let leftLimit = pad;
      let rightLimit = landingRect.width - catSize - pad;

      // Check inner container if exists
      const inner = this.landing.querySelector('.section__inner');
      if (inner) {
        const innerRect = inner.getBoundingClientRect();
        const relativeLeft = innerRect.left - landingRect.left;
        leftLimit = Math.max(leftLimit, relativeLeft);
        rightLimit = Math.min(rightLimit, relativeLeft + innerRect.width - catSize);
      }

      // If headline exists, prefer the open area below headline and above hero-stack
      if (this.headline) {
        const hRect = this.headline.getBoundingClientRect();
        const headlineBottom = hRect.bottom - landingRect.top;
        if (headlineBottom > topLimit && headlineBottom < bottomLimit - 50) {
          topLimit = headlineBottom + 12;
        }
      }

      if (this.heroStack) {
        const sRect = this.heroStack.getBoundingClientRect();
        const stackTop = sRect.top - landingRect.top;
        if (stackTop > topLimit + 40 && stackTop < bottomLimit) {
          bottomLimit = stackTop - catSize - 8;
        }
      }

      // Safeguard
      if (rightLimit <= leftLimit + 30) rightLimit = leftLimit + 30;
      if (bottomLimit <= topLimit + 30) bottomLimit = topLimit + 30;

      this.bounds = {
        minX: leftLimit,
        maxX: rightLimit,
        minY: topLimit,
        maxY: bottomLimit
      };

      // Clamp current position
      this.x = Math.max(this.bounds.minX, Math.min(this.bounds.maxX, this.x));
      this.y = Math.max(this.bounds.minY, Math.min(this.bounds.maxY, this.y));
    }

    pickNewTarget() {
      if (this.isDragging || this.isPaused) return;
      this.updateBounds();
      // Pick random destination within bounds
      this.targetX = this.bounds.minX + Math.random() * (this.bounds.maxX - this.bounds.minX);
      this.targetY = this.bounds.minY + Math.random() * (this.bounds.maxY - this.bounds.minY);
      this.state = 'WALKING';
      this.calcDirection();
    }

    // Authentic Neko98 8-direction trigonometry
    calcDirection() {
      const dx = this.targetX - this.x;
      const dy = this.targetY - this.y; // Screen coordinates (down is +)
      const length = Math.hypot(dx, dy);

      if (length === 0) {
        this.direction = 'D';
        return;
      }

      const largeX = dx;
      const largeY = -dy; // Invert Y so up is +
      const sinTheta = largeY / length;

      const sinPiPer8 = 0.3826834323651;
      const sinPiPer8Times3 = 0.9238795325113;

      if (dx > 0) {
        if (sinTheta > sinPiPer8Times3) this.direction = 'U';
        else if (sinTheta > sinPiPer8) this.direction = 'UR';
        else if (sinTheta > -sinPiPer8) this.direction = 'R';
        else if (sinTheta > -sinPiPer8Times3) this.direction = 'DR';
        else this.direction = 'D';
      } else {
        if (sinTheta > sinPiPer8Times3) this.direction = 'U';
        else if (sinTheta > sinPiPer8) this.direction = 'UL';
        else if (sinTheta > -sinPiPer8) this.direction = 'L';
        else if (sinTheta > -sinPiPer8Times3) this.direction = 'DL';
        else this.direction = 'D';
      }
    }

    startIdle() {
      if (this.isPaused) {
        this.state = 'IDLE';
        this.idleType = 'STOP';
        this.img.src = SPRITES[28];
        return;
      }

      this.state = 'IDLE';
      const rand = Math.random();

      if (rand < 0.28) {
        // Sleep for 5-9 seconds
        this.idleType = 'SLEEP';
        this.idleTimer = 300 + Math.floor(Math.random() * 240);
      } else if (rand < 0.52) {
        // Wash face
        this.idleType = 'WASH';
        this.idleTimer = 120 + Math.floor(Math.random() * 100);
      } else if (rand < 0.74) {
        // Scratch ears
        this.idleType = 'SCRATCH';
        this.idleTimer = 100 + Math.floor(Math.random() * 80);
      } else if (rand < 0.88) {
        // Yawn & sit
        this.idleType = 'YAWN';
        this.idleTimer = 80 + Math.floor(Math.random() * 60);
      } else {
        // Just sit
        this.idleType = 'STOP';
        this.idleTimer = 90 + Math.floor(Math.random() * 90);
      }
    }

    wakeUp() {
      this.state = 'AWAKE';
      this.img.src = SPRITES[0]; // 0: Awake '!' sprite
      this.idleTimer = 30; // Pause for ~0.4s with '!' before moving
    }

    loop(currentTime) {
      const delta = Math.min(currentTime - this.lastTime, 100);
      this.lastTime = currentTime;

      if (this.isVisible) {
        this.animTick++;

        if (this.state === 'DRAGGING') {
          // Dangling animation handled in pointermove
          const dragFrameIndex = Math.floor(this.animTick / 10) % 2;
          this.img.src = SPRITES[DRAG_SPRITES[dragFrameIndex]];
        } else if (this.isPaused) {
          // Permanently sitting still while paused
          this.img.src = SPRITES[28];
        } else if (this.state === 'WALKING') {
          const dx = this.targetX - this.x;
          const dy = this.targetY - this.y;
          const dist = Math.hypot(dx, dy);

          if (dist < 4) {
            this.x = this.targetX;
            this.y = this.targetY;
            this.startIdle();
          } else {
            const step = Math.min(this.speed * (delta / 16.6), dist);
            this.x += (dx / dist) * step;
            this.y += (dy / dist) * step;
            this.calcDirection();

            // Toggle movement animation frame every 14 ticks (~230ms)
            if (this.animTick % 14 === 0) {
              this.frameToggle = (this.frameToggle + 1) % 2;
              const spritePair = MOVE_SPRITES[this.direction] || MOVE_SPRITES.D;
              this.img.src = SPRITES[spritePair[this.frameToggle]];
            }
          }
        } else if (this.state === 'IDLE') {
          this.idleTimer--;

          if (this.idleType === 'SLEEP') {
            // Alternate sleep1 (30) and sleep2 with Zzz (31) every 30 ticks (~500ms)
            const sleepFrame = Math.floor(this.animTick / 30) % 2 === 0 ? 30 : 31;
            this.img.src = SPRITES[sleepFrame];
          } else if (this.idleType === 'SCRATCH') {
            // Alternate scratch1 (26) and scratch2 (27)
            const scratchFrame = Math.floor(this.animTick / 10) % 2 === 0 ? 26 : 27;
            this.img.src = SPRITES[scratchFrame];
          } else if (this.idleType === 'WASH') {
            // Alternate wash (25) and sit (28)
            const washFrame = Math.floor(this.animTick / 14) % 2 === 0 ? 25 : 28;
            this.img.src = SPRITES[washFrame];
          } else if (this.idleType === 'YAWN') {
            // Yawn (29)
            this.img.src = SPRITES[29];
          } else {
            // Stop / sit (28)
            this.img.src = SPRITES[28];
          }

          if (this.idleTimer <= 0) {
            if (this.idleType === 'SLEEP') {
              this.wakeUp();
            } else {
              this.pickNewTarget();
            }
          }
        } else if (this.state === 'AWAKE') {
          this.idleTimer--;
          this.img.src = SPRITES[0]; // Awake '!'
          if (this.idleTimer <= 0) {
            this.pickNewTarget();
          }
        }

        if (!this.isDragging) {
          this.updateElementPosition();
        }
      }

      requestAnimationFrame((t) => this.loop(t));
    }

    updateElementPosition() {
      this.el.style.transform = `translate3d(${Math.round(this.x)}px, ${Math.round(this.y)}px, 0)`;
    }
  }

  // Initialize once DOM is ready
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => new NekoLanding());
  } else {
    new NekoLanding();
  }
})();
